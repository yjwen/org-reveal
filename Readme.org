...
** MathJax
   ...
   *IMPORTANT*: Displaying equations requires internet connection to
   ...
   For local MathJax installation, download the MathJax repo and place it in the root folder. Set option =REVEAL_MATH_JAX_URL= to the URL pointing to the local MathJax.js location in the downloaded repository.
